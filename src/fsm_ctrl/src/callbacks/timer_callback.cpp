...
namespace fsm_cb
{
    void IpoptNmpcWandTotalFTimerCallback(const ros::TimerEvent& event)
    {
        ...
        {
            if(std::floor(now_cmd_time) - std::floor(last_cmd_time) > 0)
            {
    ...
    void DFBCTimerCallback(const ros::TimerEvent& event)
    {
        ...
        {
            fsm_ut::SetControllerFdb(controller);
            dfbc_controller.AttCtrl(controller.position_ref[0], controller.velocity_ref[0],
                                    controller.acceleration_ref[0], controller.attitude_ref[0],
                                    controller.position_fdb, controller.velocity_fdb);

            // mavros_msgs::AttitudeTarget att_tgt = fsm_ut::SetTargetAttAndTotalThrustCmd(dfbc_controller.quat_command.x(), dfbc_controller.quat_command.y(),
            //                                                                             dfbc_controller.quat_command.z(), dfbc_controller.quat_command.w(),
            //                                                                             dfbc_controller.thrust_percentage);
            // setpoint_raw_att_pub.publish(att_tgt);
            fsm_ctrl::dfbc_msgs dfbc_state_msgs;
            dfbc_state_msgs.header.stamp = ros::Time::now();
            dfbc_state_msgs.header.frame_id = "dfbc_state";
            dfbc_state_msgs.pos_fdb.position.x = controller.position_fdb.x();
            dfbc_state_msgs.pos_fdb.position.y = controller.position_fdb.y();
            dfbc_state_msgs.pos_fdb.position.z = controller.position_fdb.z();
