...
public:
    ...
    void StartUDPListen(uint16_t port);   // 新增：启动线程接口
    void Shutdown();                      // 新增：优雅关闭，停止定时器/线程/socket

private:
    std::thread udp_thread;       // 新增：保存线程对象，避免 detach
    std::atomic<bool> running{false}; // 新增：线程运行标志
    int sock_fd{-1};              // 新增：保存 socket 描述符，便于关闭
    std::mutex shutdown_mutex;    // 可选：保护 Shutdown 操作
};
